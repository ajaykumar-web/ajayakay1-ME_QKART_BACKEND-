...
describe("Cart test", () => {
  ...
  describe("Checkout", () => {
    ...
    it("should throw 404 error if cart is not present", async () => {
      ...
      await expect(res).rejects.toEqual(expect.objectContaining({statusCode:httpStatus.NOT_FOUND}));
    });
    ...
    it("should throw 400 error if user's cart doesn't have any product", async () => {
      ...
      expect(res).rejects.toEqual(expect.objectContaining({statusCode: httpStatus.BAD_REQUEST}));
    });
