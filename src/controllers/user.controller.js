...
const getUser = catchAsync(async (req, res) => {
  ...
  if (req.user.email != data.email) {
    throw new ApiError(httpStatus.FORBIDDEN, "User entity not matched.");
  }
  if (req.query.q === "address") {
    data = await userService.getUserAddressById(req.params.userId);
    res.status(httpStatus.OK).json({address: data.address});
  }
  else{
...
const createUser = catchAsync(async (req, res) => {
  ...
  const result = await userService.createUser(usersBody);
